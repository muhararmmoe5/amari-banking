...
export function getDb(): Database.Database {
  ...
  // Boot-time password reset — for recovering a locked-out OWNER on
  // Railway without shell access. Reads RESET_OWNER_EMAIL and
  // RESET_OWNER_PASSWORD env vars; no-op if either is unset.
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { maybeResetOwnerFromEnv } = require('./boot-password-reset');
    maybeResetOwnerFromEnv(db);
  } catch (e) {
    // eslint-disable-next-line no-console
