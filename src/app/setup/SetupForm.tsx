...
export default function SetupForm() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTx(async () => {
      const res = await setupOwnerAction(fd);
  ...
  return (
    <form className="space-y-3" onSubmit={onSubmit}>
      <label className="block">
        ...
        <input name="name" required autoFocus className="w-full" />
      </label>
      <label className="block">
        <div className="text-[11px] uppercase tracking-wider text-ink-mute mb-1">Email</div>
        <input name="email" type="email" required className="w-full" autoComplete="email" />
      </label>
      <label className="block">
